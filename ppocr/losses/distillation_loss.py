    ...
    def forward(self, predicts, batch):
        ...
        for idx, pair in enumerate(self.model_name_pairs):
            ...
            if self.num_classes > 1:
                stu_classes = stu_outs["classes"]
                tch_classes = tch_outs["classes"]
            ...
            if self.num_classes > 1:
                label_threshold_map, label_threshold_mask, label_shrink_map, label_shrink_mask, class_mask = batch[
                                                                                             1:]
            else:
                label_threshold_map, label_threshold_mask, label_shrink_map, label_shrink_mask = batch[
                                                                                                 1:]
            if self.num_classes > 1:
                loss_classes = self.loss_func(tch_classes, class_mask)
                ...
                loss_dict[k] = bce_loss + loss_binary_maps + loss_classes
            else:
                # calculate the shrink map loss
                bce_loss = self.alpha * self.bce_loss(
                    stu_shrink_maps, th_shrink_maps, label_shrink_mask)
